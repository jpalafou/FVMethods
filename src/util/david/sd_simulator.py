...
class SD_Simulator:
    def __init__(
        ...
        self.x_tp, self.dm.w_tp = gauss_legendre_quadrature(
            0.0, 1.0, self.n + 1
        )
        ...
        self.dm.deriv_sp_from_val_fp = lagrangeprime_matrix(
            self.x_sp, self.x_fp
        )
        ...
        self.mesh_cv[0] = (
            np.arange(Nx + 2)[na, :, na, na] + self.x_fp[na, na, na, :]
        ) * (boxlen_x + 2 * self.dx) / (Nx + 2) - self.dx
        self.mesh_cv[1] = (
            np.arange(Ny + 2)[:, na, na, na] + self.x_fp[na, na, :, na]
        ) * (boxlen_y + 2 * self.dy) / (Ny + 2) - self.dy
        ...
        if self.mhd:
            self.dm.affected_corner = np.zeros(
                (Ny * (n + 1) + 1, Nx * (n + 1) + 1)
            )
        ...
        self.dm.dMx = np.ndarray(
            [nvar, (Ny * (n + 1)) + 4, (Nx * (n + 1)) + 2]
        )
        self.dm.dMy = np.ndarray(
            [nvar, (Ny * (n + 1)) + 2, (Nx * (n + 1)) + 4]
        )
        self.dm.dMt = np.ndarray(
            [nvar, (Ny * (n + 1)) + 2, (Nx * (n + 1)) + 2]
        )
        self.dm.MR_face_x = np.ndarray(
            [nvar, Ny * (n + 1), (Nx * (n + 1)) + 1]
        )
        self.dm.ML_face_x = np.ndarray(
            [nvar, Ny * (n + 1), (Nx * (n + 1)) + 1]
        )
        self.dm.MR_face_y = np.ndarray(
            [nvar, Ny * (n + 1) + 1, (Nx * (n + 1))]
        )
        self.dm.ML_face_y = np.ndarray(
            [nvar, Ny * (n + 1) + 1, (Nx * (n + 1))]
        )
        ...
        self.dm.x_fp[2:-2] = (
            ...
            / Nx
            * np.hstack(
                (np.arange(Nx).repeat(n + 1) + np.tile(self.x_fp[:-1], Nx), Nx)
            )
        )
        ...
        self.dm.y_fp[2:-2] = (
            ...
            / Ny
            * np.hstack(
                (np.arange(Ny).repeat(n + 1) + np.tile(self.x_fp[:-1], Ny), Ny)
            )
        )
        ...
        if self.potential:
            ...
            self.dm.grad_phi_fv = np.ndarray(
                (2, Ny * (n + 1) + 4, Nx * (n + 1) + 4)
            )
            self.init_potential()
        ...
        if self.mhd:
            ...
            self.dm.Bx_face_x = np.ndarray(
                [(Ny * (n + 1)), (Nx * (n + 1)) + 1]
            )
            self.dm.By_face_y = np.ndarray(
                [(Ny * (n + 1)) + 1, (Nx * (n + 1))]
            )
            ...
            self.dm.Bx_fv = np.ndarray(
                [(Ny * (n + 1)) + 4, (Nx * (n + 1)) + 3]
            )
            self.dm.By_fv = np.ndarray(
                [(Ny * (n + 1)) + 3, (Nx * (n + 1)) + 4]
            )
    ...
    def compute_primitives(self, U):
        ...
        if self.mhd:
            ...
            B2 = 0.5 * (
                U[self._bx_] ** 2 + U[self._by_] ** 2 + U[self._bz_] ** 2
            )
            K = (
                0.5
                * U[0]
                * (W[self._vx_] ** 2 + W[self._vy_] ** 2 + W[self._vz_] ** 2)
            )
        else:
            B2 = 0
    ...
    def compute_fluxes(self, F, M, v_1, v_2, prims) -> None:
        ...
        F[_p_, ...] = W[v_1] * (E + pT) - B_1 * (
            B_1 * W[v_1] + B_2 * W[v_2] + B_3 * v3
        )
    ...
    def post_init(self) -> None:
        ...
        nvar = self.nvar
        W_gh = np.ndarray(
            (nvar, self.Ny + 2, self.Nx + 2, self.n + 1, self.n + 1)
        )
        ...
        if self.complex_BC:
            ...
            self.dm.c_BC_sp = np.transpose(
                self.dm.c_BC_fv.reshape(
                    self.Ny, self.n + 1, self.Nx, self.n + 1
                ),
                (0, 2, 1, 3),
            )
        ...
        if self.mhd:
            ...
            Bx_fp_x = (
                np.einsum("sf,ijfx->ijsx", self.dm.deriv_sp_from_val_fp, Az_gh)
                / self.dy
            )
            By_fp_y = (
                -np.einsum(
                    "sf,ijxf->ijxs", self.dm.deriv_sp_from_val_fp, Az_gh
                )
                / self.dx
            )
            ...
            Bx_cv = self.compute_cv_from_sp(
                self.compute_sp_from_fp(Bx_fp_x[na], 0)
            )[0]
            By_cv = self.compute_cv_from_sp(
                self.compute_sp_from_fp(By_fp_y[na], 1)
            )[0]
            W_gh[_bx_] = Bx_cv
            W_gh[_by_] = By_cv

            Bx = np.einsum(
                "mk,ijkn->ijmn", self.dm.val_cv_from_val_sp, Bx_fp_x
            )
            Bx_fv = np.ndarray(
                (
                    (self.Ny + 2) * (self.n + 1),
                    (self.Nx + 2) * (self.n + 1) + 1,
                )
            )
            Bx_fv[:, :-1] = np.transpose(
                Bx[:, :, :, :-1], (0, 2, 1, 3)
            ).reshape(
                (self.Ny + 2) * (self.n + 1), (self.Nx + 2) * (self.n + 1)
            )
            Bx_fv[:, -1] = Bx[:, -1, :, -1].reshape(
                (self.Ny + 2) * (self.n + 1)
            )

            By = np.einsum(
                "mk,ijnk->ijnm", self.dm.val_cv_from_val_sp, By_fp_y
            )
            By_fv = np.ndarray(
                (
                    (self.Ny + 2) * (self.n + 1) + 1,
                    (self.Nx + 2) * (self.n + 1),
                )
            )
            By_fv[:-1, :] = np.transpose(
                By[:, :, :-1, :], (0, 2, 1, 3)
            ).reshape(
                (self.Ny + 2) * (self.n + 1), (self.Nx + 2) * (self.n + 1)
            )
            By_fv[-1, :] = By[-1, :, -1, :].reshape(
                (self.Nx + 2) * (self.n + 1)
            )
        ...
        if self.fv_slopes == "primitives":
            M_gh = self.dm.xp.transpose(W_gh, (0, 1, 3, 2, 4)).reshape(
                nvar,
                (self.Ny + 2) * (self.n + 1),
                (self.Nx + 2) * (self.n + 1),
            )
        elif self.fv_slopes == "conservatives":
            M_gh = self.dm.xp.transpose(U_gh, (0, 1, 3, 2, 4)).reshape(
                nvar,
                (self.Ny + 2) * (self.n + 1),
                (self.Nx + 2) * (self.n + 1),
            )
        ...
        if self.advection:
            W = W_gh[:, 1:-1, 1:-1, :, :]
            W = self.dm.xp.transpose(W, (0, 1, 3, 2, 4)).reshape(
                nvar, (self.Ny) * (self.n + 1), (self.Nx) * (self.n + 1)
            )
    ...
    def init_potential(self) -> None:
        ...
        phi_sp = np.einsum(
            "km,ln,ijmn->ijkl",
            self.val_sp_from_val_cv,
            self.val_sp_from_val_cv,
            phi_cv,
        )

        phi_fp_x = np.einsum(
            "fs,zcxs->zcxf", self.dm.val_fp_from_val_sp, phi_sp
        )
        self.dm.grad_phi_sp[0] = (
            np.einsum("sf,zcxf->zcxs", self.dm.deriv_sp_from_val_fp, phi_fp_x)
            / self.dx
        )[1:-1, 1:-1, :, :]

        phi_fp_y = np.einsum(
            "fs,zcsx->zcfx", self.dm.val_fp_from_val_sp, phi_sp
        )
        self.dm.grad_phi_sp[1] = (
            np.einsum("sf,zcfx->zcsx", self.dm.deriv_sp_from_val_fp, phi_fp_y)
            / self.dy
        )[1:-1, 1:-1, :, :]

        # Now for the finite volume update
        phi_fp_x = self.dm.xp.einsum(
            "mk,ijkn->ijmn", self.dm.val_cv_from_val_sp, phi_fp_x
        )
        phi_fp_y = self.dm.xp.einsum(
            "mk,ijnk->ijnm", self.dm.val_cv_from_val_sp, phi_fp_y
        )

        phi_fv_x = np.ndarray((Ny * (n + 1) + 4, Nx * (n + 1) + 5))
        phi_fv_y = np.ndarray((Ny * (n + 1) + 5, Nx * (n + 1) + 4))

        phi_x = self.dm.xp.transpose(
            phi_fp_x[:, :, :, :-1], (0, 2, 1, 3)
        ).reshape((Ny + 2) * (n + 1), ((Nx + 2) * (n + 1)))
        phi_y = self.dm.xp.transpose(
            phi_fp_y[:, :, :-1, :], (0, 2, 1, 3)
        ).reshape((Ny + 2) * (n + 1), ((Nx + 2) * (n + 1)))
        ...
        if self.n > 1:
            ...
            phi_fv_x[..., :, -1] = phi_fp_x[:, -1, :, -1].reshape(
                (Ny + 2) * (n + 1)
            )[n - 1 : -n + 1]
            phi_fv_y[..., -1, :] = phi_fp_y[-1, :, -1, :].reshape(
                (Nx + 2) * (n + 1)
            )[n - 1 : -n + 1]
        else:
            phi_fv_x[..., :, :-1] = phi_x
        ...
        self.dm.grad_phi_fv[0] = (phi_fv_x[:, 1:] - phi_fv_x[:, :-1]) / (
            self.dm.x_fp[na, 1:] - self.dm.x_fp[na, :-1]
        )
        self.dm.grad_phi_fv[1] = (phi_fv_y[1:, :] - phi_fv_y[:-1, :]) / (
            self.dm.y_fp[1:, na] - self.dm.y_fp[:-1, na]
        )
    ...
    def compute_fp_from_sp(self, U_sp, dim) -> None:
        if dim == 0:
            U_fp = np.einsum(
                "fs,uzcxs->uzcxf", self.dm.val_fp_from_val_sp, U_sp
            )
        elif dim == 1:
            U_fp = np.einsum(
                "fs,uzcsx->uzcfx", self.dm.val_fp_from_val_sp, U_sp
            )
        return U_fp
    ...
    def compute_sp_from_fp(self, U_fp, dim) -> None:
        if dim == 0:
            U_sp = np.einsum(
                "sf,uzcxf->uzcxs", self.dm.val_sp_from_val_fp, U_fp
            )
        elif dim == 1:
            U_sp = np.einsum(
                "sf,uzcfx->uzcsx", self.dm.val_sp_from_val_fp, U_fp
            )
        return U_sp
            ...
            for ader_iter in range(self.n + 1):
                ...
                if self.detect_troubles or self.so_godunov:
                    ...
                    if self.dm.xp.any(self.dm.trouble):
                        ...
                        self.dm.F_face_x[...] = np.where(
                            self.dm.affected_face_x[na, ...] == 1,
                            self.dm.MR_face_x,
                            self.dm.F_face_x,
                        )
                        self.dm.F_face_y[...] = np.where(
                            self.dm.affected_face_y[na, ...] == 1,
                            self.dm.MR_face_y,
                            self.dm.F_face_y,
                        )
                        if self.mhd:
                            self.dm.Ez[...] = np.where(
                                self.dm.affected_corner[...] == 1,
                                self.dm.Es,
                                self.dm.Ez,
                            )
                ...
                if self.well_balance:
                    self.dm.W_cv[...] = (
                        self.compute_primitives(self.dm.U_cv + self.dm.U_eq_cv)
                        - self.dm.W_eq_cv
                    )
                else:
    ...
    def perform_iterations(self, n_step: int) -> None:
        ...
        for i in range(n_step):
            if not self.n_step % 1:
                print(
                    f"Time step #{self.n_step} (t = {self.time}, min dens = {self.dm.W_cv[0].min()}, min p = {self.dm.W_cv[self._p_].min()})"
                )
            cfl.compute_dt(self)
            self.perform_update()
        ...
        print(
            f"{self.n_step} time steps (t = {self.time}, Execution time: {round(self.execution_time,6)} s)"
        )
        ...
        while self.time < t_end:
            if nsteps != 0:
                if not self.n_step % nsteps:
                    ...
                    print(
                        f"Output {n_output} at time step #{self.n_step} (t = {self.time})"
                    )
                    self.dm.switch_to(CupyLocation.host)
                    if self.n_step == 0:
                        U = self.dm.xp.transpose(
                            self.U_init_cv, (0, 1, 3, 2, 4)
                        ).reshape(
                            self.nvar,
                            self.Ny * (self.n + 1),
                            self.Nx * (self.n + 1),
                        ...
                        U = self.dm.U_new
                    np.save(
                        self.outputs + "/rho_" + str(n_output).zfill(5), U[0]
                    )
                    np.save(
                        self.outputs + "/troubles_" + str(n_output).zfill(5),
                        self.dm.trouble,
                    )
                    self.dm.switch_to(CupyLocation.device)
            else:
                if not self.n_step % 1000:
                    print(
                        f"Time step #{self.n_step} (t = {self.time}, min dens = {self.dm.W_sp[0].min()}, min p = {self.dm.W_sp[_p_].min()})"
                    )
    ...
    def get_cv_from_Bx_face_x(self, Bx):
        ...
        self.dm.Bx_fp_x[:, :, :, :-1] = self.dm.xp.transpose(
            Bx[:, :-1].reshape(self.Ny, self.n + 1, self.Nx, self.n + 1),
            (0, 2, 1, 3),
        )
        self.dm.Bx_fp_x[:, :-1, :, -1] = self.dm.Bx_fp_x[:, 1:, :, 0]
        self.dm.Bx_fp_x[:, -1, :, -1] = Bx[:, -1].reshape(self.Ny, self.n + 1)
        self.dm.Bx_fp_x[...] = self.dm.xp.einsum(
            "mk,ijkn->ijmn", self.dm.val_sp_from_val_cv, self.dm.Bx_fp_x
        )
        return np.transpose(self.Bx_cv, (0, 2, 1, 3)).reshape(
            self.Ny * (self.n + 1), self.Nx * (self.n + 1)
        )

    def get_cv_from_By_face_y(self, By):
        # Need to find a better way to do this, it's super expensive
        self.dm.By_fp_y[:, :, :-1, :] = self.dm.xp.transpose(
            By[:-1, :].reshape(self.Ny, self.n + 1, self.Nx, self.n + 1),
            (0, 2, 1, 3),
        )
        self.dm.By_fp_y[:-1, :, -1, :] = self.dm.By_fp_y[1:, :, 0, :]
        self.dm.By_fp_y[-1, :, -1, :] = By[-1, :].reshape(self.Nx, self.n + 1)
        self.dm.By_fp_y[...] = self.dm.xp.einsum(
            "mk,ijnk->ijnm", self.dm.val_sp_from_val_cv, self.dm.By_fp_y
        )
        return np.transpose(self.By_cv, (0, 2, 1, 3)).reshape(
            self.Ny * (self.n + 1), self.Nx * (self.n + 1)
        )
    ...
    def Bx_init_sp(self):
        return np.einsum(
            "sf,cdxf->cdxs", self.dm.val_sp_from_val_fp, self.Bx_init_fp_x
        )

    @property
    def Bx_init_sp(self):
        return np.einsum(
            "sf,cdfx->cdsx", self.dm.val_sp_from_val_fp, self.By_init_fp_y
        )

    @property
    def Bx_sp(self):
        return np.einsum(
            "sf,cdxf->cdxs", self.dm.val_sp_from_val_fp, self.dm.Bx_fp_x
        )

    @property
    def By_sp(self):
        return np.einsum(
            "sf,cdfx->cdsx", self.dm.val_sp_from_val_fp, self.dm.By_fp_y
        )
    ...
    def B2_cv(self):
        if self.scheme == "sd":
            ...
            return (
                self.dm.W_cv[self._bx_] ** 2
                + self.dm.W_cv[self._by_] ** 2
                + self.dm.W_cv[self._bz_] ** 2
            )
