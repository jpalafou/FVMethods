...
def trouble_detection(s,m):
    ...
    if s.so_godunov:
        ...
        if s.NAD == "1st": # orthogonally adjacent (first neighbors)
            fv.FV_Boundaries_x(s,primitives=True,perturbation=s.detect_over_perturbations)
            W_max = compute_W_max(s.dm.M_fv,"x")
            W_min = compute_W_min(s.dm.M_fv,"x")
            fv.FV_Boundaries_y(s,primitives=True,perturbation=s.detect_over_perturbations)
            W_max = np.maximum(compute_W_max(s.dm.M_fv,"y"),W_max)[:,2:-2,2:-2]
            W_min = np.minimum(compute_W_min(s.dm.M_fv,"y"),W_min)[:,2:-2,2:-2]
        elif s.NAD == "2nd": # corners (second neighbors)
            fv.FV_Boundaries(s,primitives=True,perturbation=s.detect_over_perturbations)
        ...
        if np.any(possible_trouble) and s.n > 1:
            ...
            for var in s.limiting_vars:
                s.dm.trouble[...] = np.where(possible_trouble[var]==1,
                                                  np.where(alpha[var]<1,1,s.dm.trouble),s.dm.trouble)
        else:
            for var in s.limiting_vars:
                s.dm.trouble[...] = np.where(possible_trouble[var]==1,1,s.dm.trouble)

        # if s.complex_BC:
        if False:
            s.dm.trouble[:,:-1] = np.where((s.dm.c_BC_fv[ :,1:]-s.dm.c_BC_fv[:,:-1])<0,1,s.dm.trouble[:,:-1])
            s.dm.trouble[1:, :] = np.where((s.dm.c_BC_fv[:-1,:]-s.dm.c_BC_fv[1:, :])<0,1,s.dm.trouble[1:, :])
        ###########################
        ...
        if False:
            if s.well_balance and s.detect_over_perturbations:
                W_new += s.dm.W_eq_cv
            #For the density
            s.dm.trouble[...] = np.where(W_new[s._d_,...]>=s.min_rho,s.dm.trouble,1)
            s.dm.trouble[...] = np.where(W_new[s._d_,...]<=s.max_rho,s.dm.trouble,1)
            #For the pressure
            s.dm.trouble[...] = np.where(W_new[s._p_,...]>=s.min_P,s.dm.trouble,1)
        ...
        ##############################################
            ...
            possible_trouble_Bx = np.where(s.dm.Bx_new > Bx_max, 1, possible_trouble_Bx)
            possible_trouble_By = np.where(s.dm.By_new < By_min, 1, 0)
            possible_trouble_By = np.where(s.dm.By_new > By_max, 1, possible_trouble_By)
            if np.any(possible_trouble_Bx) or np.any(possible_trouble_By) and s.n > 0:
                s.dm.Bx_fv[2:-2,1:-1] = s.dm.Bx_new
                s.dm.By_fv[1:-1,2:-2] = s.dm.By_new
                fv.B_Boundaries(s)
                alpha = compute_smooth_extrema(s,s.dm.Bx_fv[np.newaxis,:,1:-1],"y")[0]
                s.dm.affected_face_x[...] = np.where(possible_trouble_Bx==1,np.where(alpha<1,1,0),0)
                alpha = compute_smooth_extrema(s,s.dm.By_fv[np.newaxis,1:-1,:],"x")[0]
                s.dm.affected_face_y[...] = np.where(possible_trouble_By==1,np.where(alpha<1,1,0),0)
    ...
    s.dm.affected_face_x[:,1: ] = np.where(s.dm.trouble==1,1,s.dm.affected_face_x[:,1: ])

    s.dm.affected_face_y[:-1,:] = s.dm.trouble
    s.dm.affected_face_y[1: ,:] = np.where(s.dm.trouble==1,1,s.dm.affected_face_y[1: ,:])

    if s.BC[0] == "periodic":
        s.dm.affected_face_x[:,0] = s.dm.affected_face_x[:,-1] = np.maximum(s.dm.affected_face_x[:,0],s.dm.affected_face_x[:,-1])
    if s.BC[1] == "periodic":
        s.dm.affected_face_y[0,:] = s.dm.affected_face_y[-1,:] = np.maximum(s.dm.affected_face_y[0,:],s.dm.affected_face_y[-1,:])

    if s.mhd:
        #Finally we flag the affected corner points
        s.dm.affected_corner[...]=0
        s.dm.affected_corner[:-1,:] = s.dm.affected_face_x
        s.dm.affected_corner[1: ,:] = np.where(s.dm.affected_face_x==1,1,s.dm.affected_corner[1: ,:])
        s.dm.affected_corner[:,:-1] = np.where(s.dm.affected_face_y==1,1,s.dm.affected_corner[:,:-1])
        s.dm.affected_corner[:,1: ] = np.where(s.dm.affected_face_y==1,1,s.dm.affected_corner[:,1: ])
...
def compute_min(s,A,Amin,dim):
    if dim==0:
        ...
        Amin[...,1: ] = np.where(A[...,:-1]<Amin[...,1: ],A[...,:-1],Amin[...,1: ])
        #if s.BC[0] == "periodic":    
...
def first_order_derivative(U,x_cv,dim):
    ...
    if dim==0:
        ...
        dU = (U[:,:,2:]-U[:,:,:-2])/(x_cv[na,na,2:]-x_cv[na,na,:-2])   
        
    elif dim==1:
        #dUdy(j) = [U(j+1)-U(j-1)]/[y_cv(j+1)-y_cv(j-1)]
        dU = (U[:,2:,:]-U[:,:-2,:])/(x_cv[na,2:,na]-x_cv[na,:-2,na])
    return dU
