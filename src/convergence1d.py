...
def Dt_adjustment_to_maintain_order(
    h, a, spatial_order, time_order, safety_factor=1 / 10
):
    if time_order > spatial_order:
        raise BaseException("time order is already sufficiently accurate")
    d = spatial_order - time_order
    return (safety_factor * (1 / a) * h**d) ** (1 / time_order)
...
for config in solution_configs:
    ...
    for n in global_configs["mesh sizes"]:
        ...
        print()
        # array of x-values
        x_interface = np.linspace(x_bounds[0], x_bounds[1], num=n)
        ...
        if config["time order"] > 4:
            rkorder = 4
            time_step_adjustment = (h / (x_bounds[1] - x_bounds[0])) ** (
                (config["spatial order"] - 4) / 4
            )
        ...
        if rkorder == 1:
            ...
            time_message = f"rk{rkorder}{time_step_adjustment}"
        label = (
